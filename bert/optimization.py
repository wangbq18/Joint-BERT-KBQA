...
def warmup_linear_constant(x, warmup=0.002):
    """ Specifies a triangular learning rate schedule where peak is reached at `warmup`*`t_total`-th (as provided to BertAdam) training step.
        After `t_total`-th training step, learning rate is zero. """
    if x < warmup:
        return x/warmup
    return max((x-1.)/(warmup-1.), 0.2)

SCHEDULES = {
    'warmup_cosine':   warmup_cosine,
    'warmup_constant': warmup_constant,
    'warmup_linear':   warmup_linear,
    'warmup_linear_constant': warmup_linear_constant
}
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for n, p in zip(group['names'], group['params']):
                ...
                rr = re.search("layer\.(\d{1,2})\.", n)
                if rr:
                    lr_scheduled *= 0.9 ** (12-int(rr.group(1)))
                if ".embeddings." in n:
